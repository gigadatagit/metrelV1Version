...
def seleccionar_Energia_Generada(dataFrame: pd.DataFrame, listado_Columnas: list):
    ...
    while True:
        try:
            print("Por favor, ingrese si desea visualizar las Energías Generadas para generar el informe.\n#1 - Sí\n#2 - No")
            opcion = int(input("Digite la respuesta (1 o 2): "))  # Solicita el input
            if opcion in opciones_Plantillas:
                break  # Sale del bucle si la opción es válida
            else:
                print("Opción no válida. Intente nuevamente.")
        except ValueError:
            print("Entrada no válida. Debe ingresar un número de plantilla.")

    # Evalúa la opción ingresada y genera el resultado
    if opcion == 1:
        ...
        try:
          ...
          # 'Ep1+(Med)', 'Ep2+(Med)', 'Ep3+(Med)', 'Ep1-(Med)', 'Ep2-(Med)', 'Ep3-(Med)', 'EQfund1cap+(Med)', 'EQfund2cap+(Med)', 'EQfund3cap+(Med)', 'EQfund1cap-(Med)', 'EQfund2cap-(Med)', 'EQfund3cap-(Med)', 'EQfund1ind+(Med)', 'EQfund2ind+(Med)', 'EQfund3ind+(Med)', 'EQfund1ind-(Med)', 'EQfund2ind-(Med)', 'EQfund3ind-(Med)'
          ...
          dataFrame_H_a_H_Result['KARH_IND'] = np.where(dataFrame_H_a_H_Result[nombre_Columna_Energia_Total_Inductiva] != 0, (dataFrame_H_a_H_Result[nombre_Columna_Energia_Total_Inductiva] / dataFrame_H_a_H_Result[nombre_Columna_Energia_Total_Activa] * 100), 0)

          dataFrame_H_a_H_Result['KVARH_CAP'] = np.where(dataFrame_H_a_H_Result[nombre_Columna_Energia_Total_Capacitiva] != 0, (dataFrame_H_a_H_Result[nombre_Columna_Energia_Total_Capacitiva] / dataFrame_H_a_H_Result[nombre_Columna_Energia_Total_Activa] * 100), 0)
...
def calcular_Observacion_Corriente(diccionario_Percentiles_Corriente: dict, valor_Corriente_Nominal: float):
